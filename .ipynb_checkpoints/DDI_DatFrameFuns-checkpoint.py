...
def get_param_grid(Kvals,bvals,cvals, betavals):
    ...
    which_bc_vals = list(range(0,len(cvals)))
    # make mesh
    Kmesh,which_bc_mesh,betamech,which_uinit_mesh, which_rinit_mesh = np.meshgrid(Kvals,which_bc_vals,betavals, which_uinit, which_rinit)
    
    # flatten
    [Kvec, which_bc_vec, betavec, which_unit_vec, which_rinit_vec] = [np.ndarray.flatten(item) for item in [mesh,which_bc_mesh,betamech,which_uinit_mesh, which_rinit_mesh]]
    
        
    # get b, c vecs
    bvec = np.array([bvals[i] for i in which_bc_vec])
    cvec = np.array([cvals[i] for i in which_bc_vec])
    ...
    [Kvec, bvec, cvec,betavec,u1vec,u2vec, r1vec, r2vec] = [item[whichOK] for item in [Kvec, bvec, cvec,betavec,u1vec,u2vec, r1vec, r2vec]]
    buvec = 1 - u1vec-u2vec
    pcvec = 1 - Kvec
    
    n = len(Kvec)

    data = {'K': Kvec, 'pc': pcvec,'beta': betavec,'b':bvec, 'c':cvec,  'u1init': u1vec, 'u2init':u2vec, 
    ...
    df = df.sort_values(by=['K','beta','b','c'])
